...
export async function canGenerateContent(email, contentType = 'article') {
  try {
    ...
    // Verificar si es usuario admin desde la base de datos
    try {
      const userPlan = await getUserPlanFromDB(email)
      if (userPlan && userPlan.id === 'admin') {
        console.log('✅ Usuario admin detectado desde DB:', userPlan.id)
        return { allowed: true, reason: null }
      }
    } catch (dbError) {
      console.log('Error verificando usuario admin en DB:', dbError.message)
    }
